...
const TableTemplate: FC = () => {
  ...
  const [dictLoading, { setTrue: setDictLoadingTrue, setFalse: setDictLoadingFalse }] = useBoolean(false);
...
const [curData,setCurData] = useState({})
  ...
  const renderDictStatus = (record: API.DICTMANAGEMENT) => (
    ...
    ><Switch
      ...
      onChange={() => { setDictLoadingTrue(); setDictId(record.dict_id) }}
    />
    </Popconfirm>
  );
  /**
  ...
  const columns: ProColumns<API.DICTMANAGEMENT>[] = [
    {
      ...
      render: (text) => <Space>
        <Tag
          icon={<IconFont type="icon-dict-management" className={PrimaryColor} />} >
    ...
    },
    {
      ...
      ellipsis: true,
      copyable: true,
      render: (_,record) => <Button type='link' onClick={() => handleCurData(record)}> {_}</Button>,
    },
    /* 状态 */
    {
      ...statusColumn,
  ...
  return (
    <>
      ...
      <Form form={formCode}>
        <CodeTemplate
           open={openCodeDrawer} setOpenDrawerFalse={codeDraClose} curData={curData}
        />
      </Form>
    </>
  )
