...
const UserInformation: FC<UserInformationProps> = ({ showLabel = true, disabledField = false }) => {
  const { formatMessage } = useIntl();

  const { data: roleList } = useRequest(
    async (params) => get(await getRoleList(params), 'data.list', []),
    {
  ...
  return (
    <>
      ...
      <ProFormSelect
        ...
        options={map(roleList, (r: API.ROLEMANAGEMENT) => ({
          label: r.role_name,
          value: r.role_id,
        }))}
