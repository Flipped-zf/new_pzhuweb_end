...
const LeftContent: FC = () => {
  ...
  const renderIcon = (type: string) => {
    return (
      <IconFont type={type} style={{ fontSize: 18, marginRight: 10, verticalAlign: '-5px' }} />
    );
  };
  // 获取省市区名称
  const getAreaName = () => {
  ...
  return (
    <>
      ...
      <Space direction="vertical" style={{ display: 'flex' }}>
        <Col>
          {renderIcon('icon-role-management')}
          {role_name}
        </Col>
        <Col>
          {renderIcon('icon-jobs-management')}
          {`${org_name}-${jobs_name}`}
        </Col>
        <Col>
          {renderIcon('icon-location')}
          {getAreaName()}
          {address}
        </Col>
      </Space>
