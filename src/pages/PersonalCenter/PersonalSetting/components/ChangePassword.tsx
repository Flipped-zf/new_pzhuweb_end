...
const ChangePassword: FC = () => {
  ...
  const handlerSubmit = (values: PasswordParams) => {
    ...
    if (encryptionAesPsd(values.originalPassword) !== initialState?.CurrentUser?.password) {
      message.error(
        formatMessage({ id: formatPerfix(ROUTES.PERSONALSETTING, 'change-password.error') }),
      );
    } else {
      Modal.confirm({
  ...
  return (
    <Form form={form} style={{ width: 500 }} labelCol={{ span: 4 }} onFinish={handlerSubmit}>
      ...
      <ProFormText.Password
        label={formatMessage({
          id: formatPerfix(ROUTES.PERSONALSETTING, 'change-password.original-password'),
        })}
      ...
      <Form.Item style={{ marginTop: 10 }}>
        <Button type="primary" htmlType="submit" block>
          {formatMessage({ id: INTERNATION.BUTTON_SUBMIT })}
        </Button>
      </Form.Item>
    </Form>
  );
