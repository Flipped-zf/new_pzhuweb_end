...
const SecuritySetting: FC<{ setActiveKey: React.Dispatch<React.SetStateAction<string>> }> = ({
  setActiveKey,
}) => {
  ...
  const passwordStrength = () => {
    const analysisValue: { score: number } = zxcvbn(
      decryptionAesPsd(initialState?.CurrentUser?.password || ''),
    );
    // score得分只有0~4，且只有整数范围并没有小数
    return formatMessage({
      id: `components.StrengthMeter.${keys(strengthMeterOptions)[analysisValue.score]}`,
    });
  };
  // 邮箱中间三位星号表示
  const regEmail = () => {
        ...
        metas={{
          title: {
            ...
            render: (text) => (
              <Text strong>
                {formatMessage({
                  id: formatPerfix(ROUTES.PERSONALSETTING, `security-setting.${text}`),
                })}
              </Text>
            ),
          },
          description: {
            ...
            render: (text, record, index) =>
              index > 0 ? (
                <Text type="secondary">
                  {formatMessage({
                    id: formatPerfix(ROUTES.PERSONALSETTING, `security-setting.${record.name}.tip`),
                  })}
                  ：{text}
                </Text>
              ) : null,
                ...
                <Tag key="edit" color="success">
                  {formatMessage({
                    id: formatPerfix(
                      ...
                      `security-setting.${row.name}.certified`,
                    ),
                  })}
                </Tag>
              ),
