...
const ARCard:FC = ({visible,close,ADetail}) => {
  return (
    <>
      {visible && (<div className='ARDetail' onClick={(event) => {
        if(event.target.clssname = 'ARDetail') {
            console.log(event)
            close()
        }
        ...
        <article className="movie-card" onClick={(e) => e.stopPropagation()}>
          <img src={ADetail?.posterlink} alt="Avatar wallpaper" />

          <div className="content">
            <h1>{ADetail?.title}</h1>

            <div className="infos">
                <Avatar shape="square" size="large" src={ADetail?.userInfo?.avatar_url} />


              <span>·&nbsp;&nbsp;{moment(ADetail?.created_time).format('YYYY-MM-DD')}</span>
            </div>
