...
const BlogLogs: FC<BlogLogsProps> = ({ renderSecondary }) => {
  ...
  const latestNews: LatestNewsTypes[] = [
    ...
    },
    {
      ...
      content:
        'Vue2 Admin 是一个后台管理系统解决方案，采用前后端分离技术开发。它使用了最新的技术栈，提供了丰富的功能组件，希望本项目可以帮助到您。',
      link: 'https://baiwumm.com/p/umcaj6q8',
    },
    {
      title: 'JS 中的二进制散列值和权限设计',
      content:
        '这篇文章介绍了在JavaScript中使用二进制散列值和位运算符来实现权限设计。作者讨论了权限控制的场景和常见的权限类型，并介绍了JavaScript中提供的进制表示方法和位运算符的应用。作者还提到了位运算符在传统权限系统中解决权限关联和组合的优势，并给出了使用按位或、按位与和按位非来添加、校验和剔除权限的示例。文章提到了位运算符方案的前提条件。',
